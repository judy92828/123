...
<head lang="en">
    <meta charset="UTF-8">
    <title>@if($action!='show'&& $action!='videoshow')@if(is_null($title)){{$datas['system']['webtitle']}}@else @if(!is_numeric($title)){{$title}}@if(!empty($datas['system']['webtitle']))-{{$datas['system']['webtitle']}}@endif @else {{category($title)['name']}}@if(!empty($datas['system']['webtitle']))-{{$datas['system']['webtitle']}}@endif @endif @endif @else @if(empty($info->title)){{$datas['system']['webtitle']}}@else{{$info->title}}@endif @endif
    </title>
    <meta name="keywords" content="@if($action!='show'&& $action!='videoshow') @if(empty($keywords)){{$datas['system']['keywords']}}@else{{$keywords}}@endif @else {{$info->keywords}} @endif" />
    <meta name="description" content="@if($action!='show'&& $action!='videoshow') @if(empty($description)){{$datas['system']['description']}}@else{{$description}}@endif @else {{$info->summary}} @endif"/>
    <meta name="viewport" content="user-scalable=no, width=device-width, initial-scale=1.0, maximum-scale=1.0">
    <!--引入公用样式-->
    <link rel="stylesheet" type="text/css" href="{{asset('/home/css/reset.css')}}">
    <link rel="stylesheet" type="text/css" href="{{asset('/home/css/public.css')}}">
    <!--首页样式-->
    <link rel="stylesheet" type="text/css" href="{{asset('/home/css/index.css')}}">
    <link rel="stylesheet" type="text/css" href="{{asset('/home/css/vaitalitySichuan.css')}}">
    <!--视频样式-->
    <link rel="stylesheet" type="text/css" href="{{asset('/home/css/newsContent.css')}}">
    <link rel="stylesheet" type="text/css" href="{{asset('/home/css/video.css')}}">
    <link rel="stylesheet" type="text/css" href="{{asset('/home/css/physicians.css')}}">
...
<div class="header comWidth clearfix">
    <div class="header-left clearfix">
        ...
        <div class="link"> <a href="@if(!empty($datas['system'])) {{$datas['system']->weibourl}} @endif" target="_blank">官方微博</a> <a href="@if(!empty($datas['system'])) {{$datas['system']->weixinurl}} @endif" target="_blank">官方微信</a> </div>    </div>
...
<!--引入js-->
<script src="{{asset('/home/js/jquery.slideBox.min.js')}}"></script>
<script src="{{asset('/home/js.js')}}"></script>
