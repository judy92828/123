                        ...
                        <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
                            <thead>
                                <tr role="row">
                                    ...
                                    <th class="sorting" style="text-align: center" width="200">发布管理</th></tr>
                            </thead>
                                        <tbody>
                                        @if(!empty($data))
                                            @foreach($data as $ad)
                                                <tr role="row" class="odd">
                                                    ...
                                                    <td style="text-align: center">@if($ad->status==1) 待发布 @endif</td>
                                                    <td><div class="holder" style="background: white;">
                                                            <div class="center" style="text-align: center">
                                                                <input type="checkbox" id="checkbox-10-1"><label style="box-sizing: content-box;"  for="checkbox-10-1"  onclick="changes({{$ad->id}},{{$ad->status}})"></label>
                                                            </div>
                                                        </div></td>
                                                </tr>
                                            @endforeach
...
<script type="text/javascript">
    ...
    function changes(id,state){
        $.post("{{url('admin/advert/publish')}}",{'_token':"{{csrf_token()}}","id":id,"status":state},function (data) {
            if(data.status==0){
