                        ...
                        <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
                            <thead>
                                <tr role="row">
                                    ...
                                    <th class="sorting" style="text-align: center" width="200">审核管理</th></tr>
                            </thead>
                                        <tbody>
                                        @if(!empty($data))
                                            @foreach($data as $ad)
                                                <tr role="row" class="odd">
                                                    ...
                                                    <td style="text-align: center">@if($ad->status==0) 未审核 @endif</td>
...
<script type="text/javascript">
    ...
    function changes(id,state){
        $.post("{{url('admin/advert/audit/')}}",{'_token':"{{csrf_token()}}","id":id,"status":state},function (data) {
            if(data.status==0){
