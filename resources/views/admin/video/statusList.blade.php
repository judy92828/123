...
@section('contentes')
                        ...
                        <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
                            <thead>
                                <tr role="row">
                                    ...
                                    <th class="sorting_asc" style="text-align: center" width="150">分类名</th>
    ...
    <script type="text/javascript">
        ...
        function changes(id,state){
            $.post("{{url('admin/video/editState/')}}",{'_token':"{{csrf_token()}}","id":id,"status":state},function (data) {
                if(data.status==0){
                    layer.msg(data.msg, {icon: 6});
                    setTimeout(function(){
                        window.location.reload();
