...
@section('contentes')
                                            ...
                                            @foreach($list as $vo)
                                                <tr role="row" class="odd">
                                                    ...
                                                    <td align="center"><div class="holder">
                                                            <div class="center">
                                                                <input type="checkbox" id="checkbox-10-{{$vo->id}}"><label style="box-sizing: content-box;"  for="checkbox-10-{{$vo->id}}"  onclick="changes({{$vo->id}},{{$vo->status}})"></label>
                                                            </div>
                                                        </div></td>
                                                </tr>
                                            @endforeach
    ...
    <script type="text/javascript">
        function changes(id,state){
            $.post("{{url('admin/slide/pendingrelease/')}}",{'_token':"{{csrf_token()}}","id":id,"status":state},function (data) {
                if(data.status==0){
